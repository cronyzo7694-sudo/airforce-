    ...
    <div class="card qb-list-card">
      ...
      <table class="tbl qb-tbl">
        ...
        ${slice.map((q, i) => `<tr class="qb-row" data-id="${q.id}">
          ...
          <td class="qb-q">
            <div class="qb-qtext">${AVUtil.esc(q.questionText).slice(0, 130)}${q.questionText.length > 130 ? '…' : ''}</div>
        ...
        <div class="av-modal wide">
          ...
          <div class="av-modal-body">
            ...
            <table class="qa-opt-tbl">${(q.options || []).map(o => `
              <tr class="${q.correctAnswer === o.id ? 'ok' : ''}"><td style="width:30px"><b>${o.id}.</b></td><td>${AVUtil.qtext(o.text)}</td>
