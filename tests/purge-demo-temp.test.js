/* v1.4.55 — demo-temp PURGE unit test (fake-indexeddb, no full app boot)
   v1.4.55: 4 REAL banks (12,000 Q — math 2800, reasoning 1872, english 3705,
   gs 3623; bank-meta _bundleKind ab "final"). Fresh data me demo-temp tag hai
   ...
   12,000 real + user ki q_ssc_* + airforce Q KABHI nahi chute. */
const fs = require('fs');
...
(async () => {
  ...
  t('seed: imported 12,000 (4 real banks)', rep.imported === 12000, 'got ' + rep.imported);
  t('seed: bySubject exact (math 2800 / reasoning 1872 / english 3705 / gs 3623)',
    rep.bySubject.mathematics === 2800 && rep.bySubject.reasoning === 1872 && rep.bySubject.english === 3705 && rep.bySubject.gs === 3623,
  ...
  t('survivors: 12,000 real + user 1 = 12,001 ssc Q', ssc3.length === 12001, 'got ' + ssc3.length);
  t('survivors: real bank UNTOUCHED (gs 3623, math 2800, reasoning 1872, english 3705)',
    ['gs', 'mathematics', 'reasoning', 'english'].every(s => ssc3.filter(q => q.subject === s && q.id !== 'q_ssc_mathematics_90001').length === { gs: 3623, mathematics: 2800, reasoning: 1872, english: 3705 }[s]),
  ...
  t('re-sync: count stable 12,001 ssc', all4.filter(q => q.exam === 'ssc-chsl').length === 12001);
