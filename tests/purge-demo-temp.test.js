/* v1.4.55 — demo-temp PURGE unit test (fake-indexeddb, no full app boot)
   v1.4.55: 4 REAL banks (12,212 Q — math 2824, reasoning 1872, english 3705,
   gs 3811; bank-meta _bundleKind ab "final"). Fresh data me demo-temp tag hai
   ...
   12,212 real + user ki q_ssc_* + airforce Q KABHI nahi chute. */
const fs = require('fs');
...
(async () => {
  ...
  t('seed: imported 12,212 (4 real banks)', rep.imported === 12212, 'got ' + rep.imported);
  t('seed: bySubject exact (math 2824 / reasoning 1872 / english 3705 / gs 3811)',
    rep.bySubject.mathematics === 2824 && rep.bySubject.reasoning === 1872 && rep.bySubject.english === 3705 && rep.bySubject.gs === 3811,
  ...
  t('survivors: 12,212 real + user 1 = 12,213 ssc Q', ssc3.length === 12213, 'got ' + ssc3.length);
  t('survivors: real bank UNTOUCHED (gs 3811, math 2824, reasoning 1872, english 3705)',
    ['gs', 'mathematics', 'reasoning', 'english'].every(s => ssc3.filter(q => q.subject === s && q.id !== 'q_ssc_mathematics_90001').length === { gs: 3811, mathematics: 2824, reasoning: 1872, english: 3705 }[s]),
  ...
  t('re-sync: count stable 12,213 ssc', all4.filter(q => q.exam === 'ssc-chsl').length === 12213);
